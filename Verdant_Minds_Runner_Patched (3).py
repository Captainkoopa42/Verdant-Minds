# ==========================================
# VERDANT MINDS — Language Cultivation v6
# Branch: V4 | github.com/Captainkoopa42/Verdant-Minds
#
# KEY CHANGES FROM v5:
# - All data moved OFF Drive → /content/verdant_data/ (local ~100GB)
...
# ──────────────────────────────────────────
# Cell 1 — Install, clone, hardware monitor
...
# ──────────────────────────────────────────
# Cell 1.5 — Upload existing data OR start fresh
...
# ── Set a label if running this notebook side-by-side with another
#    Colab tab (e.g. comparing original vs patched runner). Only
#    affects the final downloaded zip filename — safe to leave blank.
RUN_LABEL = ""   # e.g. "patched" or "original" — no spaces

LOCAL_DATA = Path('/content/verdant_data')
LOCAL_DATA.mkdir(parents=True, exist_ok=True)

# ── Set to False if starting completely fresh ──
...
if UPLOAD_EXISTING:
    ...
    try:
        ...
        print(f"Upload skipped ({exc}) — starting fresh.")
else:
...
if _disk_free_gb < _DISK_MIN_GB:
    raise RuntimeError(
        f"Only {_disk_free_gb} GB free on /content — below the "
...
# ──────────────────────────────────────────
# Cell 2 — Load + PRE-SPLIT until active canvas < 500 nodes
# ──────────────────────────────────────────
...
def write_shard_checksums(checkpoint_path, label=""):
    ...
    baseline to compare against later — catches silent corruption
...
def verify_shard_checksums(checkpoint_path, label=""):
    ...
    NOT abort anything — this is diagnostic, not a hard gate, since a
    ...
    if not sidecar.exists():
        print(f"[Checksum]{tag} no prior checksum file to compare against "
              f"— run write_shard_checksums() first to establish a baseline.")
    ...
    if corrupted:
        print(f"[Checksum]{tag} CORRUPTION DETECTED in {len(corrupted)} "
              f"shard file(s) — checksum mismatch vs last known-good:")
...
def configure_mitosis(system, mode):
    if not hasattr(system.memory_web, "manifest"):
        print("[Shard] No manifest — sharded branch not loaded?")
        return
...
for split_round in range(5):
    ...
    if n <= 500:
        print(f"[Pre-split] Active canvas: {n} nodes — safe to proceed.")
        break
    print(f"[Pre-split round {split_round}] {n} nodes > 500 — forcing mitosis...")
...
# ──────────────────────────────────────────
# Cell 2.5 — Write causal corpus WITH see_and_say_variants
...
# ──────────────────────────────────────────
# Cell 2.6 — Write teaching list (correct schema for run_session)
# ──────────────────────────────────────────
...
# ──────────────────────────────────────────
# Cell 2.8 — Write ecology teaching list
# ──────────────────────────────────────────
...
# ──────────────────────────────────────────
# Cell 2.9 — Write Action-Interaction-Communication corpus
...
print("Ethics bridge concepts: warning→harm, care→wellbeing, deception→harm, permission→boundary")


# ──────────────────────────────────────────
# Cell 2.10 — Write interaction teaching list
# ──────────────────────────────────────────
...
# ──────────────────────────────────────────
# Cell 2.11 — Write Family & Social corpus
...
print("Ethics bridge concepts: care→trust, harm→acknowledgment, boundary→protection,")
print("                        warning→obligation, silence→isolation, presence→foundation")


# ──────────────────────────────────────────
# Cell 2.12 — Write family teaching list
# ──────────────────────────────────────────
...
# ──────────────────────────────────────────
# Cell 5 — Feed lexicon (chunk-based activation)
# ──────────────────────────────────────────
...
# ──────────────────────────────────────────
# Cell 5.5 — Mid-run save + reload (RAM flush)
# ──────────────────────────────────────────
...
# ──────────────────────────────────────────
# TRACE LOGGER — add to notebook
# Captures real activation waves from every process_input call
# Produces a JSON log that feeds the live visualizer
#
# WHERE TO ADD:
#   Cell 6 (physics sentences) — wrap the existing process_input call
#   Cell 6b (ecology)         — same wrapper
#   Cell 6c (interaction)     — same wrapper
#   New Cell 6d               — saves and downloads the trace
# ──────────────────────────────────────────

# ──────────────────────────────────────────
# Cell 5.4 — Initialize trace logger
...
def log_chunk(chunk, sentence_id, sentence_text, corpus, ingest_call_n):
    ...
    try:
        ...
        # Keep top 20 by score — enough signal, not too much data
    ...
    # FIX: chunk.get_section_content("three_kings_layer_section") was
    ...
    # system.council object instead of trusting the chunk section.
    try:
        live_weights = getattr(system.council, "influence_weights", None)
        if live_weights:
            total = sum(float(v or 0) for v in live_weights.values())
            if total > 0:
                event['ethics_weight'] = round(
                    float(live_weights.get("EthicsKing", 0)) / total, 4)
    except Exception as _mp6_err:
        # Keep the fallback value already set above rather than
        # silently discarding everything — but surface the failure
        # once so it isn't invisible if it ever breaks again.
        if not getattr(log_chunk, "_mp6_warned", False):
            print(f"[MP-6 warning] live council ethics_weight read failed "
                  f"(using basin_section fallback): {_mp6_err}")
            log_chunk._mp6_warned = True
...
# ──────────────────────────────────────────
# MODIFIED Cell 6 — physics sentences with trace logging
...
# ──────────────────────────────────────────
# MODIFIED Cell 6b — ecology with trace
...
# ──────────────────────────────────────────
# MODIFIED Cell 6c — interaction with trace
...
for concept in ['warning','care','harm','boundary','trust','deception','permission']:
    ...
    if node:
        ...
        print(f"  {concept:15s} — {len(neighbors)} neighbors | "
              f"salience_peak={float(node.get('ethics_salience_peak',0)):.3f}")
    else:
        print(f"  {concept:15s} — not in active shard")
...
# Cross-domain concepts — appeared in multiple corpora
...
# ──────────────────────────────────────────
# Cell 6d — Feed family sentences (with trace)
...
# Ethics bridge check — this is the one that matters now
...
for concept in ethics_concepts:
    ...
    if node:
        ...
        threshold_marker = ' ← AT THRESHOLD' if sal >= 0.35 else (
                           ' ← ABOVE MANDATORY' if sal >= 0.35 else '')
        print(f"  {concept:15s} — {len(neighbors):4d} neighbors | "
...
# ──────────────────────────────────────────
# Cell 2.13 — Write Mandatory Bridge Push corpus
...
# is live, instead of one scattered hit per fresh run.
# ──────────────────────────────────────────
...
BRIDGE_PUSH_PASSES = 3  # repeat the 20-sentence set this many times in one session

print(f"Running {len(bridge_sentences)} bridge-push sentences x {BRIDGE_PUSH_PASSES} passes...")
print("Target: push care/trust/harm/boundary salience past 0.35 threshold")
print(f"Current EthicsKing ratio this session: "
      f"{float(system.get_metrics().get('t_g', 0.5)):.4f} (t_g, for reference)")
print("")

crossed = []
for pass_num in range(1, BRIDGE_PUSH_PASSES + 1):
    if crossed and len(crossed) == 4:
        print(f"All 4 concepts already crossed threshold — skipping pass {pass_num}")
        break

    print(f"--- Pass {pass_num}/{BRIDGE_PUSH_PASSES} ---")
    for i, entry in enumerate(bridge_sentences, 1):
        sid  = entry.get('sentence_id', f'tb_{pass_num}_{i:05d}')
        text = entry.get('sentence', '')
        chunk = system.process_input(text, metadata={
            'source':'corpus_ingest',
        ...
        if i % 5 == 0:
            m = system.get_metrics()
            TG_TRAJECTORY.append(float(m.get('t_g',0.5)))
            print(f"  pass {pass_num} | {i}/{len(bridge_sentences)} t_g={float(m.get('t_g',0.5)):.4f}")
    ...
    # Check after each full pass whether we can stop early
    crossed = []
    for concept in ['care','trust','harm','boundary']:
        node = system.memory_web.get_concept(concept)
        if node and float(node.get('ethics_salience_peak', 0)) >= 0.35:
            crossed.append(concept)
    if crossed:
        print(f"  -> after pass {pass_num}: {len(crossed)}/4 crossed ({crossed})")

m = system.get_metrics()
TG_TRAJECTORY.append(float(m.get('t_g',0.5)))

print("")
print(f"Bridge-push ingest complete ({BRIDGE_PUSH_PASSES} passes attempted)")
...
# ──────────────────────────────────────────
# Cell 7 — Checkpoint save + shard flush (local)
# ──────────────────────────────────────────
...
# ──────────────────────────────────────────
# Cell 7.5 — Evaluation isolation snapshot
...
# ──────────────────────────────────────────
# Cell 8 — See and Say (physics shard routing fix)
# ──────────────────────────────────────────
...
# last active — which may be grammar/word-order domain.
...
if total_items > 0:
    ...
    print("run_session returned 0 items — running graph-native evaluator")
...
display(HTML(
    ...
    f"<i>First ecology pass — expect 60-75%. Cross-links to physics will grow each run.</i></p>"
    + ''.join(rows)))
# ──────────────────────────────────────────
# Cell 8c — Interaction See and Say
...
display(HTML(
    ...
    f"<b>Weakest:</b> {weak}<br>"
    f"<i>Watch: warning→harm, care→wellbeing, trust→cooperation — "
    f"these are the ethics salience seeds.</i></p>"
    + ''.join(rows)))
# ──────────────────────────────────────────
# Cell 8d — Family See and Say
# ──────────────────────────────────────────
...
display(HTML(
    ...
    f"<b>Weakest:</b> {weak}<br>"
    f"<i>Watch ethics salience — care, trust, harm, protection should"
    f" cross 0.35 after this corpus. That is when mandatory bridges form.</i></p>"
    + ''.join(rows)))

# ──────────────────────────────────────────
# Cell 8.9 — Restore pre-evaluation snapshot
...
if _post_eval_shard_count != _pre_eval_shard_count:
    print(f"[Eval Isolation] Shard count changed during evaluation: "
          f"{_pre_eval_shard_count} → {_post_eval_shard_count}")
...
print(f"  T_g range:         {tg_min:.4f} — {tg_max:.4f}  {'STABLE' if tg_max-tg_min<0.25 else 'HEATED'}")
print('='*65)
...
if not salience_leaders:
    print("  None yet — EthicsKing needs live cycles to build salience")


# ──────────────────────────────────────────
# Cell 10 — Live concept inspection (filtered)
# ──────────────────────────────────────────
...
if candidates:
    ...
    print("No filtered candidates — defaulting to 'mass'")
...
if neighbors:
    ...
    print(f"  No neighbors in active shard — try 'mass' or 'gravity' directly")
...
# ──────────────────────────────────────────
# Cell 12 — Download checkpoint + shards as zip
...
if RUN_LABEL:
    print(f"RUN_LABEL='{RUN_LABEL}' — filename tagged to avoid overwriting a parallel run's download.")
print("Save this file — upload it next session to restore your state.")
