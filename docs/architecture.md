...
| metrics | `_metrics` |
| governance | `data_king.to_state_dict()`, `forefront_king.to_state_dict()`, `ethics_king.to_state_dict()` |
...
| `ThreeKingsCouncil.conflict_history` / `interaction_history` | council state is not included in checkpoint payload |
