...
concepts in the memory backbone. In representative runs, emergent$\to$emergent edges in a sparsified backbone often show a timing bias toward linking newer emergents to older emergents (example-run values are reported in run artifacts), suggesting temporal scaffolding during concept formation. 
...
\vspace{0.5em}
\noindent\textbf{DOI:} \href{https://doi.org/10.5281/zenodo.18870656}{10.5281/zenodo.18870656}
...
The example outputs below should be treated as run-specific illustrations.
When reporting results, include seed, provider, cycle count, and whether the run
was fresh or resumed.

\begin{itemize}[nosep]
    \item Seed: 42 (NumPy RNG + ECWF random state)
    \item Cycles: (example run; see outputs/ for exact value)
...
At the conclusion of an example run:

\begin{itemize}[nosep]
    \item Total concepts: (example run; see outputs/ for exact values)
    \item MemoryWeb backbone edges: (example run; depends on top-k setting)
    \item Access-count summaries: run-dependent example outputs
\end{itemize}
...
oriented each edge by creation order. Ordering is determined primarily from 
the \texttt{creation\_time} field stored in each emergent concept's metadata 
(a float timestamp written at concept birth); where metadata is unavailable, 
the integer timestamp suffix in the concept name is used as a fallback. Each 
EE edge is oriented newer~$\to$~older. We observed:
...
\midrule
Observed earlier-share & (example run; see outputs/) \\
Shuffle baseline (mean) & (example run; trials dependent) \\
Shuffle baseline (std) & (example run; trials dependent) \\
$z$-score & (example run) \\
\bottomrule
\end{tabular}
\caption{Temporal bias in emergent$\to$emergent link direction. The observed 
earlier-share can exceed the shuffle baseline in representative runs,
consistent with a newer$\to$older temporal bias in emergent link structure.}
...
\begin{figure}[H]
    ...
    cultivation run (example output).}
    \label{fig:results}
\end{figure}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Timestamp ordering}: Temporal ordering of emergent concepts 
          uses stored \texttt{creation\_time} metadata as the primary source 
          of truth, with the name-encoded timestamp suffix as a fallback. 
          Future work should verify consistency between these two sources 
          across all runs and flag any discrepancies.
    ...
    \item \textbf{Logging schema dependence}: analysis scripts assume current JSON
          state/log field names (for example,
          \texttt{memory\_web.memory\_store} and
          \texttt{metadata.creation\_time}). Schema drift can change reported
          metrics unless adapters are updated.
    \item \textbf{Resume effects}: resumed segments continue from persisted graph
          state and cycle history. A resumed 20-cycle segment is not equivalent
          to a fresh 20-cycle run.
    ...
    \item \textbf{Dual coherence proxies}: The system currently maintains 
          two distinct coherence measures---a wave-derived scalar 
          (cognitive temperature) and a graph-derived density proxy 
          (growth $\times$ stability $\times$ $\log N$)---which are not 
          yet formally disambiguated. Future work should assign distinct 
          names and roles to each, and clarify which is appropriate for 
          which analytical context.
    \item \textbf{Ethical distance underspecification}: The global ethical 
          distance $\Delta_E$ in the pconnect edge policy is currently 
          approximated from edge weight rather than computed from a 
          stable system-wide ethical field. Formalizing this quantity is 
          needed for the thermodynamic framing to be fully rigorous.
\end{enumerate}
