...
class TestCouncil:
    ...
    def test_council_state_roundtrip(self) -> None:
        """Council influence must survive serialize/restore (MP-5 permanent)."""
        dk, fk, ek = DataKing(), ForefrontKing(), EthicsKing()
        council = ThreeKingsCouncil(dk, fk, ek)
        council.influence_weights["EthicsKing"] = 1.35
        council.influence_weights["DataKing"] = 0.9
        council.interaction_history = [
            {"timestamp": 1.0, "conflicts": 1, "criticality": 0.5, "weights": dict(council.influence_weights)},
        ] * 12
        council.metrics["conflicts_detected"] = 3

        restored = ThreeKingsCouncil(DataKing(), ForefrontKing(), EthicsKing())
        restored.from_state_dict(council.to_state_dict())
        assert restored.influence_weights["EthicsKing"] == pytest.approx(1.35)
        assert restored.influence_weights["DataKing"] == pytest.approx(0.9)
        assert len(restored.interaction_history) == 12
        assert restored.metrics["conflicts_detected"] == 3

    def test_influence_peer_downweight_clears_salience_ceiling(self) -> None:
        """Under sustained conflict, ethics ratio must be able to exceed 0.50
        so that ratio * 0.7 activation can clear ETHICS_ANCHOR_THRESHOLD 0.35.
        """
        dk, fk, ek = DataKing(), ForefrontKing(), EthicsKing()
        council = ThreeKingsCouncil(dk, fk, ek)
        # Seed history so _update_influence is active
        for i in range(25):
            council.interaction_history.append({
                "timestamp": float(i),
                "conflicts": 2,  # avg_conflicts > 0.5
                "criticality": 0.8,
                "weights": dict(council.influence_weights),
            })
        for _ in range(200):
            council._update_influence()
        ratio = council.ethics_ratio()
        assert ratio >= 0.50, f"ethics ratio {ratio:.4f} still below 0.50"
        assert ratio * 0.7 >= 0.35, f"max salience {ratio * 0.7:.4f} still below 0.35"
