...
class VerdantKernel:
    ...
    def _validate_state(self) -> None:
        ...
        for obligation_id, obligation in self.state.obligation_kernels.items():
            ...
            if obligation.family == ObligationFamily.CONTRADICTION:
                contradiction = self.state.contradictions.get(obligation.contradiction_ref)
                if contradiction is None:
                    raise KernelInvariantError("Contradiction obligation lost its canonical defect.")
                if (
                    contradiction.claim_key != obligation.claim_key
                    or tuple(sorted(contradiction.claim_ids)) != obligation.claim_refs
                ):
                    raise KernelInvariantError("Contradiction obligation drifted from its claims.")
                if any(ref not in self.state.claims for ref in obligation.claim_refs):
